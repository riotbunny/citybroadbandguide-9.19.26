...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  let maxSpeedUI = 0;
  let minPriceUI = 999;
  let fastestProviderUI = '';
  let cheapestProviderUI = '';
  let fiberCountUI = 0;

  activeCarriers.forEach(c => {
    if (c.carrier.name.toLowerCase().includes('fiber') || c.carrier.name === 'AT&T' || c.carrier.name === 'Google Fiber') fiberCountUI++;
    c.carrier.plans.forEach(p => {
      const spd = Math.max(p.downloadSpeed || 0, p.uploadSpeed || 0);
      if (spd > maxSpeedUI) { maxSpeedUI = spd; fastestProviderUI = c.carrier.name; }
      if (p.price > 0 && p.price < minPriceUI) { minPriceUI = p.price; cheapestProviderUI = c.carrier.name; }
    });
  });
  if (minPriceUI === 999) minPriceUI = 39.99;
  if (!fastestProviderUI) fastestProviderUI = activeCarriers[0]?.carrier.name || 'Local Providers';
  if (!cheapestProviderUI) cheapestProviderUI = activeCarriers[0]?.carrier.name || 'Local Providers';
...
export default async function LocationPage({ params, searchParams }: Props) {
  ...
  const activeSort = searchParamsAwaited?.sort || 'recommended';
